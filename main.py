...
# in_memory=True - SQLite qulflanishini 100% oldini oladi
...
# ==================== 🛠️ JONLI DIAGNOSTIKA LOGERI ====================
@app.on_message(filters.group)
async def debug_group_messages(client, message):
    chat_title = message.chat.title or "Nomsiz guruh"
    chat_id = message.chat.id
    sender = message.from_user.first_name if message.from_user else "Tizim"
    # Har qanday guruh xabarini Render logiga yozib chiqaradi
    print(f"💬 [DIAGNOSTIKA] Guruhdan xabar keldi! Guruh: '{chat_title}' | ID: {chat_id} | Kimdan: {sender} | Matn: '{message.text}'")
# =====================================================================

# 1. Navbatdagi buyurtmalar ro'yxati (/list)
    ...
    while True:
        ...
        for job in pending_jobs[:]:
            if now_uz >= job["time"]:
                ...
                try:
                    contact = await app.import_contacts([InputPhoneContact(phone=phone, first_name=f"Mijoz {phone}")])
                    if contact.users:
            ...
            if (now_uz - info["sent_time"]).total_seconds() > 600 and not info["reminded"]:
                try:
                    if info["state"] == "waiting_location":
                        reminder_text = "Iltimos, ertangi tantana uchun joylashuv manzilini yuborishingizni kutyapmiz. 😊🎬📍"
                    else:
                        reminder_text = "Iltimos, kuyov uydan soat nechida chiqishini yozib yuborishingizni kutyapmiz. 😊⏱️🤵"
                    await app.send_message(user_id, reminder_text)
                    info["reminded"] = True
                    await app.send_message(GROUP_ID, f"⏳ **Eslatma yuborildi:** Mijoz hali javob bermadi. Qayta eslatma ketdi.", reply_to_message_id=info["msg_id"])
                except Exception as e:
                    pass
...
# Standart Muvaffaqiyatli ishga tushish funksiyasi (Idle)
async def main():
    print("🔄 Userbot ishga tushirilmoqda...")
    await app.start()
    try:
        me = await app.get_me()
        print(f"✅ USERBOT TELEGRAMGA MUVAFFAQIYATLI ULANDI!")
        print(f"👤 Akkaunt: {me.first_name} (@{me.username or 'username_yoq'})")
        print(f"⚙️ Sozlangan Guruh ID: {GROUP_ID}")
    except Exception as e:
        print(f"⚠️ Diagnostika xatosi: {e}")
    
    # Schedulerni alohida fondagi task qilib yoqamiz
    asyncio.create_task(scheduler_loop())
    print("🚀 Bot to'liq faol! Guruhdan xabarlar kutilmoqda...")
